...
The Melanin Core daemon can be configured to act as a trusted "border
router", implementing the melanin wire protocol and relay, making
...
The ZeroMQ feature in Melanin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building melanind:
...
For instance:
    ...
    $ melanind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/melanind.tx.raw \
...
These options can also be provided in melanin.conf.
...
From the perspective of melanind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into melanind directly. Furthermore, no information is
...
using. Melanind appends an up-counting sequence number to each
